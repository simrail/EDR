...
const TrainTimetableTimeline: React.FC<Props> = ({ itemIndex, closestStationIndex, isAtTheStation , renderOnlyLine = false}) => {
    return (
        <>
            {itemIndex !== 0 && (
                <>
            ...
            {!renderOnlyLine && <>
                <div className={`absolute w-4 h-4 ${itemIndex <= closestStationIndex ? 'bg-slate-500 dark:bg-slate-200' : 'bg-slate-300 dark:bg-slate-600'} left-1 rounded-full top-1/2 -translate-y-1/2 border-2 ${itemIndex % 2 ? 'border-gray-50 dark:border-gray-700' : 'border-white dark:border-gray-800' } z-10`}></div>
                {isAtTheStation && (
