...
function getTrueTrainNumber(pageContent, expectedTrainNumber) {
    return pageContent.includes(expectedTrainNumber)
}
    ...
    for (let i = 0; i < allTrainNumbers.length - 1; i++) {
        ...
        try {
            ...
            for (let z = 0; z < SCRAPPER_TRAINS_MAX_ROWS; z++) {
                ...
                const isTrainNumberAccurate = getTrueTrainNumber(trainRouteContent, trainNumber)
                if (!isTrainNumberAccurate) {
                    console.error("Train number mismatch, braking loop !");
                    break;
                } else {
